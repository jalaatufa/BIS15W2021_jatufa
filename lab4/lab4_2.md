...
```r
#filter(name of the data frame, column name == the row you're looking for)
```
...
```r
#filter(fish, near function (specify the variable, ))
```
...
#filter(dataframe, between (variable, ))
```

## Using `filter()` on multiple conditions
#just because it works, doesn't mean its right, use checks
